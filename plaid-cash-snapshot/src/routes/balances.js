 ...
 router.get('/balances/summary', async (req, res, next) => {
   try {
     ...
     for (const item of itemsResult.rows) {
       try {
         ...
         for (const account of balanceData.accounts) {
           ...
           const accountResult = await db.query(accountQuery, [account.account_id, user_id]);
         ...
         logger.error(`Failed to fetch balances for item ${item.item_id}:`, itemError);
       }
     }
     ...
     if (summaryResult.rows.length > 0) {
       ...
       summary = await this.calculateManualSummary(allBalances, balanceTimestamp);
     }
     ...
     const response = {
       ...
       data: {
         ...
         credit_cards_total_owed: parseFloat(summary.credit_cards_total_owed || 0),
